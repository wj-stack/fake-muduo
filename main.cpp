...
void memoryTest()
{
    ...
    cout << size << endl;
    size_t size1 = memoryTools.SecondSearch<int>(&MemoryTools::isEqual<int>, (&numA));
...
//    Person["name"] = "tony";
//    Person["age"] = 22;
//    spdlog::info(Person.toStyledString());
...
//        std::vector<map<std::string,std::string> > vec;
    ...
    httpServer.Get("/read", [&](RequestHeader &requestHeader, ResponseHeader &responseHeader) {
        cout << &a << endl;
        int pid = lexical_cast<int>(requestHeader.getParam("pid", to_string(g_pid)));
        spdlog::info("get Param address: {}", requestHeader.getParam("address", ToHex((long long) &a)));
        auto address = ToDec<long long>(requestHeader.getParam("address", ToHex((long long)&a)));
...
#undef XX
    ...
    httpServer.Get("/write", [&](RequestHeader &requestHeader, ResponseHeader &responseHeader) {
        cout << &a << endl;
        int pid = lexical_cast<int>(requestHeader.getParam("pid", to_string(g_pid)));
        auto address = ToDec<long long>(requestHeader.getParam("address", ToHex((long)&a)));
        std::string type = requestHeader.getParam("type", "int");
